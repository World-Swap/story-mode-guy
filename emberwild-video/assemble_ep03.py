#!/usr/bin/env python3
"""Assemble EMBERWILD Episode Three from its 42 shots.

Same pipeline as Episode Two: the shots are generated with generateSound off,
so a continuous ambient bed is harvested from Episode 1's clips and laid under
the whole cut. That is deliberately better than per-shot generated audio, which
restarts its ambience every five seconds.
"""
...
def ambient_bed(seconds, out):
    ...
    # Ep3 lives on an open windy moor at night, so the bed leans on Ep1's night and
    # wind-heavy clips rather than the camp bustle Ep2 used.
    picks = [srcs[i] for i in (30, 0, 12, 33, 26) if i < len(srcs)]   # night, forest wind, drum, low fire, crowd
    if not picks:
        return None
...
def main():
    ep = json.loads((HERE / "shots_ep03.json").read_text())
    ...
    silent = HERE / "_ep03_silent.mp4"
    build(clips, captions, silent, title="EMBERWILD",
          subtitle="Episode Three — Old Wounds")

    dur = probe(silent)
    bed = ambient_bed(dur, HERE / "_tmp" / "bed.m4a")
    out = HERE / "EMBERWILD_Ep03_Old-Wounds.mp4"
    if bed:
