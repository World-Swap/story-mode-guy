...
| Asset | File |
|---|---|
| Avatar | `editions/01-misty-rainforest/avatars/avatar-waterfall.png` |
| Banner | `editions/01-misty-rainforest/banners/banner-valley.jpg` |

Built from edition 01's frames by `avatar.py` and `banner.py`. Channel art is
deliberately not remade per edition — it should stay stable so the channel
stays recognisable.
